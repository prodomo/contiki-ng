...
/****************************************************/
int modbusRead(unsigned char* data)
{
  int i;
  int datalen;

  memcpy(data, rs485_buffer, sizeof(rs485_buffer));
  datalen = rs485_input_len();

  if(datalen <= 0) {
    printf("modbusRead RX Error...length <= 0\n");
    return -1;
  }
  else{

    printf("modbusRead Read data (%d):", datalen);
    for(i = 0; i < datalen; i++) {
      printf("%02x", rs485_buffer[i]);
    }
    printf("\n\r");
...
/****************************************************/
int modbusSendResponse(unsigned char *data, unsigned char dataLen,
        unsigned char hasCRC)
{
  unsigned int crc;
  int sendStatus, i;
